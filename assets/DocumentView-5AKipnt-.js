...
| Fee configuration | Not affected. It can still be pushed to the corresponding client normally. |
...
Therefore, no statistics in the backend does not mean the client is not running, and it does not mean fee configuration was not delivered.

## Does Configuration Still Work?

The core fee configuration feature is not affected by this issue. You can still maintain the corresponding CID, coin, wallet, ratio, device name, and pool address configuration in Custom Management, and the backend can still push those settings to the corresponding client.

The limitation is visibility: you may be able to confirm configuration delivery and client behavior, but you may not see complete hashrate, online status, or chart data in the current Custom Management backend.
...
3. When explaining it to the customer, clarify that this is a statistics reporting ownership issue and does not affect the core fee configuration feature.
...
| Fee configuration | Не затрагивается. Конфигурацию можно нормально отправлять соответствующему client. |
...
Поэтому отсутствие статистики в backend не означает, что client не работает, и не означает, что fee configuration не была доставлена.

## Будет ли работать конфигурация?

Основная функция fee configuration не затрагивается этой проблемой. Вы по-прежнему можете управлять в Custom Management соответствующим CID, coin, wallet, ratio, device name и pool address, а backend может нормально отправлять эти настройки соответствующему client.

Ограничение касается только видимости: вы можете подтвердить доставку конфигурации и поведение client, но можете не видеть полный hashrate, online status и chart data в текущем Custom Management backend.
...
3. При объяснении клиенту подчеркните, что это проблема ownership отправки статистики, а не проблема основной fee configuration.
...
| 抽水配置 | 不受影响，仍可以正常推送到对应客户端。 |

## 为什么会这样

RustMinerSystem 客户端在服务器上建立统计归属后，统计数据会优先沿用最早安装版本对应的上报关系。也就是说，如果这台服务器最早接入的是官方版或其他 CID 定制版，后续再安装您的定制版时，统计数据仍可能继续归属到原来的版本。

因此，后台“看不到统计数据”不代表客户端没有运行，也不代表抽水配置没有下发。

## 配置是否仍然生效

核心的抽水配置功能不受该问题影响。您仍然可以在定制管理后台维护对应 CID、币种、钱包、比例、设备名和矿池地址等配置，后台也可以正常把配置推送到对应客户端。

需要注意的是，您只能确认配置推送和客户端执行结果，无法在当前定制管理后台中看到完整的算力、在线状态和统计图表。

## 排查建议

1. 先确认客户服务器是否曾经安装过官方版或其他 CID 定制版。
2. 如果后台没有统计数据，但配置可以正常下发，应优先判断为统计归属仍在最早安装版本名下。
3. 与客户沟通时，可以说明这是统计上报归属问题，不影响核心抽水配置功能。
