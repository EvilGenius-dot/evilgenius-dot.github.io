...
Therefore, no statistics in the backend does not mean the client is not running, and it does not mean the inside fee configuration from Service Management was not delivered.

## Does Configuration Still Work?

The core inside fee configuration feature is not affected by this issue. You can still maintain the corresponding CID, coin, ratio, device name, pool address, port, and protocol configuration in Custom Management, and the backend can still push those settings to the corresponding client.

The limitation is visibility: you may be able to confirm configuration delivery and client behavior, but you may not see complete hashrate, online status, fee hashrate, or chart data in the current Custom Management backend. The inside fee configuration itself should still be checked from \`Edit Fee Rate\` in \`Service Management\`.
...
3. When explaining it to the customer, clarify that this is a statistics reporting ownership issue and does not affect the core inside fee configuration feature.
...
Поэтому отсутствие статистики в backend не означает, что client не работает, и не означает, что inside fee configuration из Service Management не была доставлена.

## Будет ли работать конфигурация?

Основная функция inside fee configuration не затрагивается этой проблемой. Вы по-прежнему можете управлять в Custom Management соответствующим CID, coin, ratio, device name, pool address, port и protocol, а backend может нормально отправлять эти настройки соответствующему client.

Ограничение касается только видимости: вы можете подтвердить доставку конфигурации и поведение client, но можете не видеть полный hashrate, online status, fee hashrate и chart data в текущем Custom Management backend. Саму inside fee configuration нужно проверять через \`Edit Fee Rate\` в \`Service Management\`.
...
3. При объяснении клиенту подчеркните, что это проблема ownership отправки статистики, а не проблема основной inside fee configuration.
...
因此，后台“看不到统计数据”不代表客户端没有运行，也不代表服务管理里的内抽配置没有下发。

## 配置是否仍然生效

核心的内抽配置功能不受该问题影响。您仍然可以在定制管理后台维护对应 CID、币种、比例、设备名、矿池地址、端口和协议等配置，后台也可以正常把配置推送到对应客户端。

需要注意的是，您只能确认配置推送和客户端执行结果，无法在当前定制管理后台中看到完整的算力、在线状态、手续费算力和统计图表。内抽配置本身仍应在 \`服务管理\` 页的 \`编辑费率\` 弹窗中核对。

## 排查建议

1. 先确认客户服务器是否曾经安装过官方版或其他 CID 定制版。
2. 如果后台没有统计数据，但配置可以正常下发，应优先判断为统计归属仍在最早安装版本名下。
3. 与客户沟通时，可以说明这是统计上报归属问题，不影响核心内抽配置功能。
