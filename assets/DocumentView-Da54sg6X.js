...
Custom Management is the operations console for every installed RustMinerSystem custom build under your account. It is not a single customer client or an installer. It is the central console used by operators to manage CIDs, client installs, active devices, coin permissions, inside fee configuration, audits, and account visibility.

![Custom Management dashboard](/image/docs/custom-management/features/zh/dashboard.png)

_Dashboard: an administrator view shows aggregate client count, active devices, regional distribution, and coin share across the authorized custom-build scope._

## Core Purpose

| Module                | Purpose                                                                                                                                      |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Dashboard             | View client count, active devices, world distribution, coin share, external wallet-fee hashrate, and coin-level charts for all visible custom builds. |
| Inside Fee Configuration | Maintain inside fee rules by CID and coin from the Edit Fee Rate dialog in Service Management, hot-update running RustMiner clients, and apply settings to historical versions when needed. |
| Coin Permission Management | Maintain which coins each custom RustMiner build is allowed to create from Coin Permission Configuration in Service Management. |
| iOS Widget            | Generate a Scriptable script for paid custom editions of RustMinerSystem so customers can view summary statistics on the iPhone home screen. |
...
The console uses CID as the primary management unit. A CID usually represents a delivered custom build or a customer-side management scope. Operators can inspect install scale, active devices, coin permissions, and inside fee settings by CID.
...
| iOS widget script       | A Scriptable script with a CTL Token inserted, used to read summary statistics for the matching custom edition on the iPhone home screen. |
| Coin permission         | The list of coins a custom RustMiner build can create.                                                                                    |
| Inside fee configuration | Per-coin fee routing settings such as ratio, device name, pool address, port, and protocol.                                              |
...
1. Dashboard, service list, and client list are used for daily observation.
2. Coin permissions and inside fee configuration are production configuration changes.
...
| Coin | Switch between \`All\` and a specific coin for hashrate, external wallet-fee hashrate, maps, and charts. |
...
\`Fee Hashrate\` is used to observe hashrate diverted by the customer's RustMinerSystem fee configuration. It is not the official fee billing ratio.
...
| Fee Hashrate | Live hashrate produced by the selected coin's RustMinerSystem fee configuration. |

Client and active-device metrics are useful for deployment scale. Total hashrate and fee hashrate are best checked at single-coin granularity against the customer's RustMinerSystem and upstream pool data. To check inside fee rules configured in Custom Management, open \`Edit Fee Rate\` in \`Service Management\` for the matching CID and coin.
...
| View | Use |
| --- | --- |
| Hashrate chart | Compare total hashrate and external wallet-fee hashrate over time for the selected coin. |
| Active-device chart | Inspect active-device changes for the selected coin. |

When a customer reports missing hashrate, missing fee hashrate, or dropped devices, first filter by management scope and coin, then compare the chart around the reported time. If the report is about inside fee ratio or inside-fee revenue, check \`Edit Fee Rate\`, version activation scope, and the customer's network environment.

## Daily Check

1. Check \`Clients\` and \`Active Devices\`.
2. Review the coin share to confirm primary coins did not shift unexpectedly.
3. Select key coins and review live hashrate, external wallet-fee hashrate, and charts.
4. If one region dropped, continue with client details or Service Management checks.
5. If only one custom build is affected, filter to that CID and repeat the check.
`,"./en/custom-management/guide/03-inside-fee-configuration.md":`---
id: insideFeeConfiguration
slug: inside-fee-configuration
title: Inside Fee Configuration
navTitle: Inside Fees
description: Configure inside fee rules for different coins from the Edit Fee Rate dialog in Service Management, including hot updates, historical version activation, and customer network checks.
category: features
categoryTitle: Feature Usage
categoryOrder: 2
order: 3
...
`,"./en/custom-management/guide/04-coin-permissions.md":`---
id: coinPermissionManagement
slug: coin-permissions
title: Coin Permission Management
navTitle: Coin Permissions
description: Maintain coin creation permissions for installed custom RustMiner builds from Coin Permission Configuration in Service Management.
category: features
categoryTitle: Feature Usage
categoryOrder: 2
order: 4
...
`,"./en/custom-management/guide/06-ios-widget.md":`---
id: iosWidget
slug: ios-widget
title: Custom iOS Widget
navTitle: iOS Widget
description: Paid custom editions of RustMinerSystem can use a Scriptable-based iOS widget to show online devices, pending reviews, client count, coin device totals, hashrate, and trends on the iPhone home screen.
category: features
categoryTitle: Feature Usage
categoryOrder: 2
order: 6
...
![Custom iOS widget preview](/image/custom-ios-widget/preview.jpg)

_The widget shows online devices, pending reviews, client count, coin device totals, hashrate, and trends on the iPhone home screen._
...
`,"./en/custom-management/guide/07-no-statistics-data.md":`---
id: noStatisticsData
slug: no-statistics-data
title: No Statistics Data
navTitle: No Statistics Data
description: Explains why hashrate, online, and chart data may not appear in Custom Management when a customer server previously installed the official build or another CID custom build, and whether fee configuration is affected.
category: faq
categoryTitle: FAQ
categoryOrder: 4
order: 7
...
If a customer server first installed the official RustMinerSystem build, or first installed a custom build that belongs to another CID, and then later installs your custom build, statistics may still be reported to the version that was installed first.

In this situation, your Custom Management backend may not show statistics for that server, including hashrate statistics, online statistics, client counts, and chart data.

## Impact Scope

| Item | Impact |
| --- | --- |
| Hashrate statistics | May not appear in your Custom Management backend. |
| Online statistics | May not be counted under your custom build. |
| Statistics charts | Coin hashrate, online trend, client trend, and similar charts may have no data. |
| External wallet fee configuration | Not affected. It can still be pushed to the corresponding client normally. |

## Why This Happens

After the RustMinerSystem client establishes a statistics ownership relationship on a server, statistics continue to prefer the reporting relationship created by the earliest installed version. In other words, if the server first connected through the official build or another CID custom build, later installing your custom build may not move the statistics ownership to your backend.

Therefore, no statistics in the backend does not mean the client is not running, and it does not mean the external wallet fee configuration from Service Management was not delivered.

## Does Configuration Still Work?

The core external wallet fee configuration feature is not affected by this issue. You can still maintain the corresponding CID, coin, wallet, ratio, device name, and pool address configuration in Custom Management, and the backend can still push those settings to the corresponding client.

The limitation is visibility: you may be able to confirm configuration delivery and client behavior, but you may not see complete hashrate, online status, external fee hashrate, or chart data in the current Custom Management backend. Pool-node inside fees should still be checked through the independent \`Pool Nodes\` view.

## Troubleshooting Suggestions

1. Confirm whether the customer server previously installed the official build or another CID custom build.
2. If the backend has no statistics but configuration can be delivered normally, treat it first as a statistics ownership issue under the earliest installed version.
3. When explaining it to the customer, clarify that this is a statistics reporting ownership issue and does not affect the core external wallet fee configuration feature.
4. For new servers, ask customers to install the custom build that should own the statistics first, so the statistics ownership does not attach to another version.
...
- [Program Cannot Start](/document/rustminersystem/program-cannot-start)
- [Admin Console Is Unreachable](/document/rustminersystem/admin-unreachable)
...
`,"./en/rustminersystem/guide/17-transparent-proxy.md":`---
id: transparentProxy
slug: transparent-proxy
title: RustMinerSystem Transparent Proxy
navTitle: Transparent Proxy
description: Learn how RustMinerSystem transparent proxy works, how it differs from plain forwarding, and how to build an RMS to jump RustMinerSystem to downstream RustMinerSystem path.
category: features
categoryTitle: Feature Usage
categoryOrder: 4
order: 17
...
2. Set the listening port that local RMS will connect to.
3. Set the primary pool address to the downstream RustMinerSystem RMS port, such as \`next-rustminer.example.com:9000\`.
...
1. Confirm the downstream RustMinerSystem RMS port is running and can connect to the pool.
2. Start the transparent proxy port on the jump RustMinerSystem server.
3. Change local RMS to use the jump server address and transparent proxy port.
4. Confirm local RMS connects successfully and the jump server connection count increases.
5. Check miners, hashrate, and connection logs on the downstream RustMinerSystem server.
...
- [Fee wallet has no revenue](/document/rustminersystem/fee-wallet-no-revenue)
- [Hashrate loss](/document/rustminersystem/hashrate-loss)
`,"./en/rustminersystem/guide/19-program-cannot-start.md":`---
id: program-cannot-start
slug: program-cannot-start
title: Program Cannot Start
navTitle: Program cannot start
description: Fix DNS resolution issues when RustMinerSystem fails to fetch server config during startup with dns error or failed to lookup address information.
category: troubleshooting
categoryTitle: Troubleshooting
categoryOrder: 3
order: 1
...
navTitle: Overview
description: Install RustMinerSystem CLI from the main RustMinerSystem installer and learn the rustminer command, web access switching, and common operations.
category: getting-started
categoryTitle: Getting Started
categoryOrder: 1
order: 1
...
- After confirmation, the script sets \`ENABLE_WEB_UI\` to \`0\`; the web dashboard becomes unreachable and statistics are viewed from the CLI.
- To restore web access, run the install script again and choose the web access menu item.
- The CLI file is installed at \`/root/rustminersystem/RustMinerSystemCLI-0.1.0\` by default.
- The command launcher is installed at \`/usr/local/bin/rustminer\` by default.
`,"./en/rustminersystem-cli/guide/02-configuration.md":`---
id: configuration
slug: configuration
title: Change RustMinerSystem Configuration In CLI Mode
navTitle: Configuration
description: Edit rust-config, port-config.json, and wallet-config.json in RustMinerSystem CLI mode, then apply changes with rustminer restart.
category: getting-started
categoryTitle: Getting Started
categoryOrder: 1
order: 2
...
Custom Management - это операционная консоль для всех установленных custom-сборок RustMinerSystem, доступных вашему аккаунту. Это не отдельный клиент и не установщик, а центральная панель для CID, установленных клиентов, активных устройств, прав по монетам, inside fee configuration, аудита и видимости аккаунтов.

![Панель Custom Management](/image/docs/custom-management/features/zh/dashboard.png)

_Панель администратора показывает агрегированные клиенты, активные устройства, регионы и долю монет в разрешенной области custom-сборок._

## Назначение

| Модуль                | Назначение                                                                                                                               |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Dashboard             | Клиенты, активные устройства, карта регионов, доля монет, external wallet-fee hashrate и графики по всем видимым custom-сборкам.          |
| Inside Fee Configuration | Настройка inside fee по CID и монете через Edit Fee Rate в Service Management, hot update для работающих RustMiner и применение к historical versions при необходимости. |
| Coin Permission Management | Управление монетами, которые каждая custom RustMiner build может создавать, через Coin Permission Configuration в Service Management. |
| iOS Widget            | Генерация Scriptable-скрипта для платных custom-версий RustMinerSystem, чтобы клиент видел краткую статистику на домашнем экране iPhone. |
...
| iOS widget script       | Scriptable-скрипт со вставленным CTL Token, который читает краткую статистику нужной custom-версии на iPhone. |
| Coin permission         | Список монет, которые custom RustMiner build может создавать.                                                 |
| Inside fee configuration | Настройки fee routing по монете: ratio, device name, pool address, port и protocol.                          |
...
1. Dashboard, service list и client list используются для наблюдения.
2. Coin permissions и inside fee configuration являются production-изменениями.
...
| Coin | Переключение между \`All\` и конкретной монетой для hashrate, external wallet-fee hashrate, карты и графиков. |
...
\`Fee Hashrate\` используется для наблюдения hashrate, который отводится fee configuration на стороне RustMinerSystem клиента. Это не official fee billing ratio.
...
| Fee Hashrate | Live hashrate, созданный fee configuration выбранной монеты в RustMinerSystem. |

Client и active-device метрики подходят для оценки масштаба установки. Total hashrate и fee hashrate лучше проверять по одной монете, сравнивая данные RustMinerSystem клиента и upstream pool. Для проверки inside fee из Custom Management откройте \`Edit Fee Rate\` в \`Service Management\` для нужного CID и монеты.
...
| Вид | Назначение |
| --- | --- |
| Hashrate chart | Сравнение total hashrate и external wallet-fee hashrate по времени. |
| Active-device chart | Динамика активных устройств выбранной монеты. |

При жалобах на hashrate, fee hashrate или offline devices сначала фильтруйте по CID и монете, затем смотрите время на графике. Если вопрос касается inside fee ratio или дохода inside fee, проверяйте \`Edit Fee Rate\`, область применения версии и сеть клиента.

## Ежедневная проверка

1. Проверьте \`Clients\` и \`Active Devices\`.
2. Посмотрите долю монет.
3. Выберите ключевые монеты и проверьте live hashrate, external wallet-fee hashrate и графики.
4. Если проблема в регионе, переходите к client details или Service Management.
5. Если затронут один custom build, отфильтруйте его CID и повторите проверку.
`,"./ru/custom-management/guide/03-inside-fee-configuration.md":`---
id: insideFeeConfiguration
slug: inside-fee-configuration
title: Настройка inside fee
navTitle: Inside Fee
description: Настройка inside fee для разных монет через Edit Fee Rate в Service Management: hot update, historical versions и проверка сети клиента.
category: features
categoryTitle: Использование функций
categoryOrder: 2
order: 3
...
# Настройка inside fee

Inside fee настраивается в \`Service Management\` - \`Actions\` - \`Edit Fee Rate\`. Оператор может управлять правилами inside fee по CID custom-сборки и монете, чтобы разные монеты использовали разные настройки fee routing.

## Где находится настройка

1. Откройте \`Service Management\`.
2. Найдите нужный custom service.
3. Нажмите \`Actions\` в этой строке.
4. Откройте \`Edit Fee Rate\`.
5. Выберите монету и добавьте или измените inside fee configuration.

Перед сохранением подтвердите CID, имя custom-сборки и монету, чтобы не применить настройку к неправильной custom build.

## Как применяется

После сохранения inside fee configuration работающие custom RustMiner клиенты синхронизируют fee configuration через hot update. В обычной ситуации RustMiner не нужно останавливать, а работающие у клиента майнеры не нужно перезапускать.

То есть уже работающие майнеры клиента не должны прерывать добычу из-за изменения inside fee в backend. RustMiner продолжит работу после синхронизации новой конфигурации.

## Historical versions

Важно учитывать область действия версии: новая inside fee configuration по умолчанию применяется только к latest version.

Если клиент использует старую версию RustMiner, одного сохранения в \`Edit Fee Rate\` недостаточно. В \`Service Management\` откройте \`Actions\` нужного сервиса, используйте \`Apply to Historical Versions\`, выберите historical version клиента и отправьте, чтобы старые клиенты использовали эту конфигурацию.

Рекомендуемый порядок:

1. Сначала настройте inside fee для latest version в \`Edit Fee Rate\`.
2. Подтвердите фактическую версию RustMiner у клиента.
3. Если клиент работает на старой версии, откройте \`Apply to Historical Versions\`.
4. Выберите нужную historical version и отправьте.
5. Затем проверьте поведение client и данные на стороне upstream pool клиента.

## Проверка сети клиента

При добавлении или изменении inside fee configuration нельзя проверять только ratio. Нужно оценить, сможет ли настройка реально работать в сети клиента.

Проверьте особенно внимательно:

| Проверка | Описание |
| --- | --- |
| Pool address | Может ли сервер клиента подключиться к настроенному pool domain или IP. |
| Port and protocol | Разрешены ли выбранный port и TCP/SSL protocol сетью клиента, firewall или политикой дата-центра. |
| DNS resolution | Если используется domain, стабильно ли он резолвится в окружении клиента. |
| Regional limits | Есть ли в регионе клиента ограничения доступа, блокировка маршрута или высокая задержка. |
| Existing customer setup | Не конфликтует ли новая inside fee configuration с текущим pool, proxy или network policy клиента. |

Если настроенный адрес недоступен из сети клиента, backend может сохранить конфигурацию успешно, но фактический inside fee routing не будет работать как ожидается.

## Рекомендации

- Перед добавлением или изменением inside fee configuration подтвердите версию RustMiner у клиента.
- Для клиентов на старых версиях обязательно используйте \`Apply to Historical Versions\`.
- Перед изменением ratio, pool address, port или protocol оцените, поддерживает ли это сеть клиента.
- Для важных клиентов или сложных сетей сначала проверьте конфигурацию на малой области, затем расширяйте на все устройства.
- Если клиент сообщает, что настройка не сработала, сначала проверьте activation version, доступность сети, protocol и блокировку port.
`,"./ru/custom-management/guide/04-coin-permissions.md":`---
id: coinPermissionManagement
slug: coin-permissions
title: Управление правами монет
navTitle: Права монет
description: Управление правами создания монет для установленных custom RustMiner builds через Coin Permission Configuration в Service Management.
category: features
categoryTitle: Использование функций
categoryOrder: 2
order: 4
...
# Управление правами монет

Coin permission management находится в \`Service Management\` - \`Actions\` - \`Coin Permission Configuration\`. Эта настройка определяет, какие монеты установленная custom RustMiner build может создавать.

После сохранения RustMiner clients при создании монеты проверяют эти permissions и решают, разрешено ли текущей custom build создавать выбранную монету.

## Где находится настройка

1. Откройте \`Service Management\`.
2. Найдите custom service, для которого нужно изменить permissions.
3. Нажмите \`Actions\` в этой строке.
4. Откройте \`Coin Permission Configuration\`.
5. Включите или отключите монеты, которые эта custom build может создавать.
6. Сохраните конфигурацию.

Перед операцией подтвердите CID и имя custom-сборки, чтобы не включить или отключить права для неправильной custom build.

## Что контролируют permissions

Coin permissions определяют список монет, доступных при создании монеты в client. Для одной custom build монеты, включенные в backend, будут доступны для создания в RustMiner. Монеты, не включенные в backend, не будут доступны для coin creation.

| Состояние | Поведение client |
| --- | --- |
| Enabled | RustMiner client может использовать эту монету при создании. |
| Disabled | RustMiner client не имеет права создавать эту монету. |

Эта настройка отвечает за то, можно ли создать монету. Она не задает inside fee ratio и не заменяет inside fee configuration.

## Область действия

Coin permission configuration применяется к установленным custom RustMiner builds. Оператор может отдельно вести список разрешенных монет по CID для разных клиентов или custom-сборок.

Типовые сценарии:

- Открыть нужные монеты для новой custom build.
- Закрыть монеты, которые custom build пока не должна создавать.
- Вести разные permission scopes для разных CID.
- Перед запуском новой монеты у клиента проверить, что custom build уже имеет нужное permission.

## Рекомендации

- Перед открытием новой монеты подтвердите, что клиенту действительно нужна возможность ее создавать.
- Перед отключением монеты подтвердите, что это не заблокирует будущие потребности клиента.
- Если несколько CID требуют одинаковые permissions, все равно проверяйте целевую область по одному.
- Coin permissions определяют только право создания. Для изменения fee-routing rules используйте \`Edit Fee Rate\` и inside fee configuration.
...
`,"./ru/custom-management/guide/06-ios-widget.md":`---
id: iosWidget
slug: ios-widget
title: Custom iOS widget
navTitle: iOS Widget
description: Платные custom-версии RustMinerSystem могут использовать iOS widget на базе Scriptable для просмотра online-устройств, ожидающих проверок, числа клиентов, устройств по монетам, хешрейта и трендов на домашнем экране iPhone.
category: features
categoryTitle: Использование функций
categoryOrder: 2
order: 6
...
![Превью custom iOS widget](/image/custom-ios-widget/preview.jpg)

_Widget показывает online-устройства, ожидание проверки, число клиентов, устройства по монетам, хешрейт и тренды на домашнем экране iPhone._
...
`,"./ru/custom-management/guide/07-no-statistics-data.md":`---
id: noStatisticsData
slug: no-statistics-data
title: Нет статистических данных
navTitle: Нет статистики
description: Объясняет, почему hashrate, online status и charts могут не отображаться в Custom Management, если на сервере клиента раньше была установлена официальная версия или custom build другого CID, и влияет ли это на fee configuration.
category: faq
categoryTitle: Частые вопросы
categoryOrder: 4
order: 7
---
...
Если на сервере клиента сначала была установлена официальная версия RustMinerSystem или custom build, принадлежащий другому CID, а затем клиент установил вашу custom build, статистика может по-прежнему отправляться в версию, которая была установлена первой.

В этой ситуации ваш Custom Management backend может не показывать статистику по этому серверу: hashrate statistics, online statistics, client count и данные графиков.

## Область влияния

| Объект | Влияние |
| --- | --- |
| Hashrate statistics | Могут не отображаться в вашем Custom Management backend. |
| Online statistics | Могут не засчитываться в online count вашей custom build. |
| Statistics charts | Графики coin hashrate, online trend, client trend и похожие charts могут быть пустыми. |
| External wallet fee configuration | Не затрагивается. Конфигурацию можно нормально отправлять соответствующему client. |

## Почему это происходит

После того как RustMinerSystem client создает на сервере связь для отправки статистики, данные статистики обычно продолжают использовать reporting relationship самой ранней установленной версии. Иначе говоря, если сервер сначала подключился через официальную версию или custom build другого CID, последующая установка вашей custom build может не перенести ownership статистики в ваш backend.

Поэтому отсутствие статистики в backend не означает, что client не работает, и не означает, что external wallet fee configuration из Service Management не была доставлена.

## Будет ли работать конфигурация?

Основная функция external wallet fee configuration не затрагивается этой проблемой. Вы по-прежнему можете управлять в Custom Management соответствующим CID, coin, wallet, ratio, device name и pool address, а backend может нормально отправлять эти настройки соответствующему client.

Ограничение касается только видимости: вы можете подтвердить доставку конфигурации и поведение client, но можете не видеть полный hashrate, online status, external fee hashrate и chart data в текущем Custom Management backend. Pool-node inside fee нужно проверять через независимую страницу \`Pool Nodes\`.

## Рекомендации по проверке

1. Уточните, была ли на сервере клиента раньше установлена официальная версия или custom build другого CID.
2. Если в backend нет статистики, но конфигурация доставляется нормально, сначала рассматривайте это как ownership problem статистики у самой ранней установленной версии.
3. При объяснении клиенту подчеркните, что это проблема ownership отправки статистики, а не проблема основной external wallet fee configuration.
4. Для новых серверов попросите клиента сначала устанавливать ту custom build, которая должна владеть статистикой, чтобы ownership статистики не закрепился за другой версией.
...
- [Программа не запускается](/ru/document/rustminersystem/program-cannot-start)
- [Админ-панель недоступна](/ru/document/rustminersystem/admin-unreachable)
...
`,"./ru/rustminersystem/guide/17-transparent-proxy.md":`---
id: transparentProxy
slug: transparent-proxy
title: Прозрачный прокси RustMinerSystem
navTitle: Прозрачный прокси
description: Как работает прозрачный прокси RustMinerSystem, чем он отличается от обычной пересылки и как построить цепочку RMS -> jump RustMinerSystem -> нижний RustMinerSystem.
category: features
categoryTitle: Использование функций
categoryOrder: 4
order: 17
...
- [Нет дохода fee-кошелька](/ru/document/rustminersystem/fee-wallet-no-revenue)
- [Потеря хешрейта](/ru/document/rustminersystem/hashrate-loss)
`,"./ru/rustminersystem/guide/19-program-cannot-start.md":`---
id: program-cannot-start
slug: program-cannot-start
title: Программа не запускается
navTitle: Программа не запускается
description: Исправление DNS resolution, если RustMinerSystem при запуске не может получить server config и показывает dns error или failed to lookup address information.
category: troubleshooting
categoryTitle: Диагностика
categoryOrder: 3
order: 1
...
Если RustMinerSystem успешно импортирует конфигурационный файл, но затем не может получить server configuration, а в логе есть \`dns error\` или \`failed to lookup address information\`, обычно программа не читает корректную DNS-конфигурацию сервера.

## Симптомы

Лог запуска может выглядеть так:
...
Это проблема DNS resolution. Когда RustMinerSystem использует сетевой стек Rust для DNS, он читает resolver configuration из \`/etc/resolv.conf\` и не продолжает искать DNS через upstream temporary resolver configuration.

Так как \`/etc/resolv.conf\` часто является временным DNS configuration file, программа может не резолвить \`api.rustpool.pro\` даже после корректной настройки DNS в системе, если \`/etc/resolv.conf\` не указывает на реально действующую resolver configuration.

## Восстановление

После проверки корректной DNS-настройки сервера укажите \`/etc/resolv.conf\` на активную конфигурацию systemd-resolved:
...
Перезапустите RustMinerSystem и проверьте, появляются ли в startup log сообщения \`Get Server Config Failed\` и \`dns error\`.
...
navTitle: Обзор
description: Установка RustMinerSystem CLI через основной installer RustMinerSystem, команда rustminer, переключение web-доступа и базовые операции.
category: getting-started
categoryTitle: Начало работы
categoryOrder: 1
order: 1
...
RustMinerSystem CLI опубликован через основной install-скрипт RustMinerSystem. Он подходит для операторов, которые не хотят открывать web-панель управления и предпочитают смотреть статистику или выполнять базовые действия из терминала сервера либо jump host.

## Команда установки

Запустите команду от root на Linux-сервере:

\`\`\`bash
bash <(curl -s -L https://github.com/EvilGenius-dot/RustMinerSystem/raw/main/install.sh)
\`\`\`

Это та же команда, которая используется для установки основного сервера RustMinerSystem. После запуска скрипта выберите язык и продолжите из меню:

| Пункт меню | Назначение                                                                                         |
| ---------- | -------------------------------------------------------------------------------------------------- |
| 1          | Установить основную программу RustMinerSystem. Используйте сначала, если сервер еще не установлен. |
| 22         | Установить command-line tool, скачать \`RustMinerSystemCLI-0.1.0\` и создать launcher \`rustminer\`.   |
| 23         | Удалить command-line tool, CLI-файлы и launcher, которыми управляет скрипт.                        |
| 10 / 11    | Включить или отключить web-доступ. CLI-режим требует отключенного web-доступа.                     |
| 21         | Показать runtime status, web port, статус CLI tool и пути установки.                               |

## Команды после установки

Скрипт создает launcher в \`/usr/local/bin/rustminer\`. Частые команды:
...
Запустите \`rustminer\` без аргументов, чтобы открыть CLI tool. Используйте \`start\`, \`stop\` или \`restart\`, чтобы управлять основным сервисом RustMinerSystem.

## Важные примечания

- Перед установкой CLI скрипт предупреждает, что web-доступ RustMinerSystem должен быть отключен.
- После подтверждения скрипт задает \`ENABLE_WEB_UI=0\`; web-панель станет недоступна, а статистика будет просматриваться через CLI.
- Чтобы восстановить web-доступ, снова запустите install-скрипт и выберите пункт управления web-доступом.
- CLI-файл по умолчанию устанавливается в \`/root/rustminersystem/RustMinerSystemCLI-0.1.0\`.
- Launcher команды по умолчанию устанавливается в \`/usr/local/bin/rustminer\`.
`,"./ru/rustminersystem-cli/guide/02-configuration.md":`---
id: configuration
slug: configuration
title: Изменение конфигурации RustMinerSystem в CLI-режиме
navTitle: Конфигурация
description: Редактирование rust-config, port-config.json и wallet-config.json в CLI-режиме RustMinerSystem с применением через rustminer restart.
category: getting-started
categoryTitle: Начало работы
categoryOrder: 1
order: 2
...
定制管理后台用于集中管理自己名下所有已经安装的 RustMinerSystem 定制版程序。它不是单个客户终端，也不是安装器，而是面向运营方的总控后台：把各个定制版的 CID、客户端安装量、活跃设备、币种权限、内抽配置和审核流程放在同一个入口中。

![定制管理后台仪表盘](/image/docs/custom-management/features/zh/dashboard.png)

_仪表盘：管理员视角可以看到授权范围内定制版本的客户端数量、活跃设备、地区分布和币种占比。_

## 核心定位

| 模块       | 作用                                                                                  |
| ---------- | ------------------------------------------------------------------------------------- |
| 仪表盘     | 汇总所有可见定制版的客户端数量、活跃设备、世界分布、币种占比、外抽手续费算力和币种折线图。 |
| 内抽配置   | 在服务管理页的编辑费率弹窗中维护各 CID/币种的内抽规则，支持运行中的 RustMiner 热更新，并可应用到历史版本。 |
| 币种权限管理 | 在服务管理页的币种权限配置中维护定制版可创建的币种范围。 |
| iOS 小组件 | 为付费定制版 RustMinerSystem 生成 Scriptable 脚本，让客户在 iPhone 桌面查看简易统计。 |
| 审核列表   | 处理钱包新增、编辑和删除申请，让高风险配置经过审核确认后再生效。                      |
| 权限分配   | 维护账号之间的管理层级，控制不同管理账号能看到和操作哪些定制版。                      |

## 管理对象

后台以 CID 作为核心管理单位。一个 CID 对应一个已经交付并安装的定制版程序或客户侧管理范围。运营方可以按 CID 查看安装量、活跃设备、币种权限和内抽配置。
...
| iOS 小组件脚本 | 写入 CTL Token 的 Scriptable 脚本，用于在 iPhone 桌面读取对应定制版的简易统计。 |
| 币种权限       | 控制定制版 RustMiner 创建币种时可以使用哪些币种。                              |
| 内抽配置       | 控制定制版在指定币种下的抽水比例、设备名、矿池地址、端口和协议等策略。          |
...
1. 仪表盘、服务列表和客户端列表用于日常观察。
2. 币种权限和内抽配置属于配置修改，提交前必须确认 CID、币种、版本、比例和客户网络环境。
...
| 查看币种 | 在 \`全部\` 和具体币种之间切换，决定算力折线图、外抽手续费算力和世界算力的币种口径。 |

如果选择 \`全部\` 币种，页面仍会展示客户端数量、活跃设备和设备占比，但实时算力、外抽手续费算力和币种折线图需要先选择具体币种。

这里的 \`手续费算力\` / \`手续费（外抽实时）\` 用于观察客户服务器 RustMinerSystem 中抽水配置实际分流出的算力，不代表官方费率账单口径。
...
| 手续费（外抽实时） | 选择具体币种后显示当前币种由 RustMinerSystem 抽水配置产生的实时抽水算力。 |

客户端数量和活跃设备适合观察定制版部署规模；总算力和外抽手续费算力适合在单币种维度上对照客户 RustMinerSystem 与矿池侧数据。如果要核对管理后台配置的内抽规则，请到 \`服务管理\` 页的 \`编辑费率\` 弹窗查看对应 CID 和币种。
...
| 视图 | 用途 |
| --- | --- |
| 算力折线图 | 查看所选币种的总算力和外抽手续费算力趋势。 |
| 活跃设备折线图 | 查看所选币种的活跃设备数量变化。 |

建议在客户反馈“算力少了”“外抽没看到”“设备掉线”时，先按管理权限和币种过滤，再对照折线图中的时间点。如果客户反馈的是内抽比例或内抽收益，应回到 \`编辑费率\` 中核对配置、版本生效范围和客户网络环境。
...
- 哪些币种是当前定制体系的主要设备来源。
- 新开币种是否已经有设备接入。
- 某个币种占比突然变化时，是否需要到服务管理中的内抽配置或币种权限继续检查。
...
4. 如果某个地区设备下降明显，记录国家/地区后到客户端明细或服务管理页继续排查。
5. 如果只影响某一个定制版，在 \`管理权限\` 中切换到对应 CID 再复查。
`,"./zh/custom-management/guide/03-inside-fee-configuration.md":`---
id: insideFeeConfiguration
slug: inside-fee-configuration
title: 内抽配置
navTitle: 内抽配置
description: 在服务管理页的编辑费率弹窗中为不同币种配置内抽规则，了解配置热更新、历史版本生效和客户网络环境检查要点。
category: features
categoryTitle: 功能使用
categoryOrder: 2
order: 3
...
内抽配置位于 \`服务管理\` 页的 \`操作\` - \`编辑费率\` 弹窗中。运营方可以按定制版 CID 和币种维护对应的内抽配置，让不同币种使用不同的抽水规则。
...
配置前请先确认目标 CID、定制名称和币种，避免把配置提交到错误的定制版。

## 生效方式

内抽配置保存后，正在运行的定制版 RustMiner 会同步热更新抽水配置。正常情况下，不需要停止 RustMiner，也不需要重启客户正在运行的矿机。

也就是说，客户现场已经在跑的矿机不会因为后台调整内抽配置而中断挖矿；RustMiner 会在后台同步到新的抽水配置后继续工作。

## 历史版本生效

需要特别注意，配置完成后的内抽默认只对最新版本生效。

如果客户正在运行的是旧版本 RustMiner，仅在 \`编辑费率\` 中保存配置还不够。需要在 \`服务管理\` 页对应服务的 \`操作\` 中使用 \`应用配置至历史版本\`，选择客户正在使用的历史版本并提交后，旧版本客户端才会使用这份配置。

建议流程：

1. 先在 \`编辑费率\` 中完成最新版本的内抽配置。
2. 确认客户实际运行的 RustMiner 版本。
3. 如果客户运行旧版本，进入 \`应用配置至历史版本\`。
4. 选择对应历史版本并提交生效。
5. 后续再观察客户端和客户矿池侧表现是否符合预期。

## 网络环境检查

新增或编辑内抽配置时，不能只看比例是否正确，还必须结合客户所在网络环境判断配置能否实际生效。

请重点确认：

| 检查项 | 说明 |
| --- | --- |
| 矿池地址 | 客户服务器是否能够访问配置中的矿池域名或 IP。 |
| 端口与协议 | 客户网络、防火墙、机房策略是否允许对应端口和 TCP/SSL 协议。 |
| DNS 解析 | 如果使用域名，客户环境中的 DNS 是否能稳定解析。 |
| 地区限制 | 客户所在地区是否存在访问限制、线路阻断或延迟异常。 |
| 客户原有配置 | 新内抽配置是否和客户原有矿池、代理或网络策略冲突。 |

如果配置在客户网络中无法访问，即使后台保存成功，实际抽水也可能无法按预期生效。

## 操作建议

- 新增、编辑内抽配置前，先确认客户使用的 RustMiner 版本。
- 对旧版本客户，不要忘记执行 \`应用配置至历史版本\`。
- 调整比例、矿池地址、端口或协议前，先评估客户网络是否支持。
- 对重要客户或复杂网络环境，建议先小范围验证配置可用性，再扩大到全部设备。
- 配置保存后，如果客户反馈无效，优先检查版本是否已生效、网络是否可达、协议和端口是否被拦截。
`,"./zh/custom-management/guide/04-coin-permissions.md":`---
id: coinPermissionManagement
slug: coin-permissions
title: 币种权限管理
navTitle: 币种权限管理
description: 在服务管理页的币种权限配置中维护已安装定制版的币种创建权限，决定 RustMiner 客户端创建币种时可选择的范围。
category: features
categoryTitle: 功能使用
categoryOrder: 2
order: 4
...
币种权限管理位于 \`服务管理\` 页的 \`操作\` - \`币种权限配置\` 中。这里用于编辑已安装定制版 RustMiner 的币种创建权限。

配置保存后，RustMiner 客户端在创建币种时，会根据这里的权限配置判断当前定制版是否允许创建对应币种。
...
3. 在该行点击 \`操作\`。
4. 打开 \`币种权限配置\`。
5. 勾选或取消勾选允许该定制版创建的币种。
6. 保存配置。

操作前请先确认目标 CID 和定制名称，避免把币种权限开放或关闭到错误的定制版。

## 权限作用

币种权限控制的是客户端创建币种时的可选范围。对于同一个定制版，后台允许的币种会出现在 RustMiner 客户端可创建的币种范围内；后台未允许的币种，客户端在创建币种时将不具备对应权限。

| 配置状态 | 客户端表现 |
| --- | --- |
| 已开启 | RustMiner 客户端创建币种时可以使用该币种。 |
| 未开启 | RustMiner 客户端创建币种时不具备该币种创建权限。 |

这项配置用于控制“能不能创建某个币种”，不用于设置内抽比例，也不替代内抽配置。

## 适用范围

币种权限配置面向所有已安装的定制版 RustMiner。运营方可以按 CID 分别维护不同客户或不同定制版的币种创建范围。

常见场景：

- 为新交付的定制版开放指定币种。
- 关闭某个定制版暂时不允许创建的币种。
- 按客户业务范围分别维护不同 CID 的币种权限。
- 在客户新增业务前，提前确认该定制版是否已经具备对应币种权限。

## 操作建议

- 开放新币种前，确认客户确实需要该币种创建能力。
- 关闭币种权限前，确认不会影响客户后续创建需求。
- 多个 CID 需要相同币种权限时，也要逐一确认目标范围，避免误操作。
- 币种权限只决定创建权限；如果还需要调整抽水规则，请到 \`编辑费率\` 中维护内抽配置。
...
`,"./zh/custom-management/guide/06-ios-widget.md":`---
id: iosWidget
slug: ios-widget
title: 定制版 iOS 小组件
navTitle: iOS 小组件
description: 付费定制版 RustMinerSystem 可以使用基于 Scriptable 的 iOS 小组件，在 iPhone 桌面查看在线设备、待审核、客户端数量、币种设备数、算力和趋势。
category: features
categoryTitle: 功能使用
categoryOrder: 2
order: 6
...
付费定制版 RustMinerSystem 可以使用基于 Scriptable 的 iOS 小组件。它是配套付费定制版提供的手机桌面入口，用于让客户快速查看自己定制版的简易统计。

![定制版 iOS 小组件预览](/image/custom-ios-widget/preview.jpg)

_小组件会在 iPhone 桌面展示在线设备、待审核、客户端数量、币种设备数、算力和趋势。_

## 适用对象

| 对象       | 说明                                                                |
| ---------- | ------------------------------------------------------------------- |
| 定制服务方 | 手工生成 CTL Token，并通过下载页生成写入 token 的 Scriptable 脚本。 |
| 定制版客户 | 在 iPhone 安装 Scriptable，导入脚本后把小组件添加到桌面。           |
| 管理后台   | 提供小组件读取统计数据所需的定制管理接口。                          |

## 展示内容

小组件面向快速巡检，不替代完整的定制管理后台。它会尽量把客户最关心的数据压缩到 iOS 桌面尺寸中：

| 数据       | 说明                                                   |
| ---------- | ------------------------------------------------------ |
| 在线设备   | 当前定制版在线或活跃设备数量。                         |
| 待审核     | 当前仍需要处理的钱包或配置审核数量。                   |
| 客户端数量 | 当前定制版已安装客户端数量。                           |
| 币种列表   | 按设备数量展示主要币种，例如 BTC、LTC、ETC、ZEC、XMR。 |
| 算力与趋势 | 展示每个币种的简易算力值和短趋势线。                   |

## 生成脚本

1. 为对应客户手工生成 CTL Token，并确认 token 只对应这个客户的定制版统计权限。
2. 打开 \`下载\` 中的 \`定制版 iOS 小组件\` 页面。
3. 在页面中粘贴 CTL Token，点击生成并下载脚本。
4. 页面会在浏览器本地把 token 写入脚本里的 \`CTL_TOKEN\` 变量，并下载新的 JavaScript 文件。

Token 不会提交到服务器，也不会写入网站仓库。它只会存在于本次生成的脚本文件中。

## 客户安装流程

1. 在 iPhone 的 App Store 安装 Scriptable。
2. 将生成好的 JavaScript 文件发送到 iPhone。
3. 在文件分享菜单中选择 Scriptable 导入脚本；如果分享菜单不可用，也可以在 Scriptable 中新建脚本并粘贴文件内容。
4. 长按 iPhone 桌面，点击加号，选择 Scriptable 的大号小组件。
5. 编辑小组件，选择刚导入的脚本。
6. 等待小组件刷新，确认数据显示的是该客户自己的定制版数据。

## 安全与维护

- CTL Token 等同于小组件读取统计数据的凭证，只发给对应客户。
- 如果 token 泄露，应重新生成 token，并让客户重新下载脚本。
- iPhone 必须能访问定制管理接口，否则小组件无法刷新。
- 客户更换手机、删除 Scriptable 或重新安装脚本时，需要重新导入带 token 的脚本文件。
- 小组件用于桌面概览，配置修改、审核处理和权限调整仍应在定制管理后台完成。
`,"./zh/custom-management/guide/07-no-statistics-data.md":`---
id: noStatisticsData
slug: no-statistics-data
title: 无统计数据
navTitle: 无统计数据
description: 说明客户服务器先安装过官方版或其他 CID 定制版后，再安装当前定制版时，为什么定制管理后台可能看不到算力、在线和图表统计，以及抽水配置是否受影响。
category: faq
categoryTitle: 常见问题
categoryOrder: 4
order: 7
...
如果客户服务器曾经先安装过官方版 RustMinerSystem，或先安装过其他 CID 的定制版本，之后再安装您的定制版本，统计数据可能仍会上报到最先安装的版本名下。

这种情况下，您的定制管理后台可能看不到该服务器的统计数据，例如算力统计、在线统计、客户端统计，以及各类统计图表数据。

## 影响范围

| 项目 | 影响 |
| --- | --- |
| 算力统计 | 可能不会显示在您的定制管理后台中。 |
| 在线统计 | 可能不会计入您的定制版本在线数量。 |
| 统计图表 | 币种算力、在线趋势、客户端趋势等图表可能没有数据。 |
| 外抽钱包配置 | 不受影响，仍可以正常推送到对应客户端。 |

## 为什么会这样

RustMinerSystem 客户端在服务器上建立统计归属后，统计数据会优先沿用最早安装版本对应的上报关系。也就是说，如果这台服务器最早接入的是官方版或其他 CID 定制版，后续再安装您的定制版时，统计数据仍可能继续归属到原来的版本。

因此，后台“看不到统计数据”不代表客户端没有运行，也不代表服务管理里的外抽钱包配置没有下发。

## 配置是否仍然生效

核心的外抽钱包配置功能不受该问题影响。您仍然可以在定制管理后台维护对应 CID、币种、钱包、比例、设备名和矿池地址等配置，后台也可以正常把配置推送到对应客户端。

需要注意的是，您只能确认配置推送和客户端执行结果，无法在当前定制管理后台中看到完整的算力、在线状态、外抽手续费算力和统计图表。矿池节点内抽仍按 \`矿池节点\` 页面的独立口径核对。

## 排查建议

1. 先确认客户服务器是否曾经安装过官方版或其他 CID 定制版。
2. 如果后台没有统计数据，但配置可以正常下发，应优先判断为统计归属仍在最早安装版本名下。
3. 与客户沟通时，可以说明这是统计上报归属问题，不影响核心外抽钱包配置功能。
4. 对新服务器，建议客户优先安装需要纳入统计的定制版本，避免统计归属落到其他版本。
...
- [程序无法启动](/zh/document/rustminersystem/program-cannot-start)
- [后台无法访问](/zh/document/rustminersystem/admin-unreachable)
...
`,"./zh/rustminersystem/guide/17-transparent-proxy.md":`---
id: transparentProxy
slug: transparent-proxy
title: RustMinerSystem 透明代理
navTitle: 透明代理
description: 了解 RustMinerSystem 透明代理的工作方式、与纯转发的区别，以及 RMS 到 RustMinerSystem 跳板机再到下一层 RustMinerSystem 的配置方法。
category: features
categoryTitle: 功能使用
categoryOrder: 4
order: 17
...
- [费率钱包没有收益](/zh/document/rustminersystem/fee-wallet-no-revenue)
- [算力损耗](/zh/document/rustminersystem/hashrate-loss)
`,"./zh/rustminersystem/guide/19-program-cannot-start.md":`---
id: program-cannot-start
slug: program-cannot-start
title: 程序无法启动
navTitle: 程序无法启动
description: RustMinerSystem 启动时获取服务端配置失败，并出现 dns error 或 failed to lookup address information 时的 DNS 解析处理方式。
category: troubleshooting
categoryTitle: 常见故障
categoryOrder: 3
order: 1
...
navTitle: 概览
description: 使用 RustMinerSystem 主安装脚本安装 CLI，了解 rustminer 命令、WEB 访问切换和常用运维入口。
category: getting-started
categoryTitle: 开始使用
categoryOrder: 1
order: 1
...
RustMinerSystem CLI 已通过 RustMinerSystem 主安装脚本发布。CLI 适合不想开放 Web 管理后台、需要在服务器终端或跳板机中查看统计与执行基础操作的部署场景。

## 安装命令

在 Linux 服务器中使用 root 用户执行：

\`\`\`bash
bash <(curl -s -L https://github.com/EvilGenius-dot/RustMinerSystem/raw/main/install.sh)
\`\`\`

该命令与 RustMinerSystem 主程序安装命令相同。进入脚本后先选择语言，再根据菜单继续：

| 菜单项  | 用途                                                                        |
| ------- | --------------------------------------------------------------------------- |
| 1       | 安装 RustMinerSystem 主程序。未安装主程序时，需要先执行此项。               |
| 22      | 安装命令行工具，下载 \`RustMinerSystemCLI-0.1.0\` 并创建 \`rustminer\` 启动器。 |
| 23      | 卸载命令行工具，清理脚本管理的 CLI 文件和启动器。                           |
| 10 / 11 | 开启或关闭 WEB 访问。CLI 模式要求 WEB 访问处于关闭状态。                    |
| 21      | 查看运行状态、WEB 端口、CLI 工具状态和安装路径。                            |

## 安装后的命令

脚本会在 \`/usr/local/bin/rustminer\` 创建命令启动器。常用命令如下：
...
不带参数运行 \`rustminer\` 会打开 CLI 工具。带 \`start\`、\`stop\`、\`restart\` 参数时，可启动、停止或重启 RustMinerSystem 主服务。

## 重要说明

- 安装 CLI 前，脚本会提示需要关闭 RustMinerSystem 的 WEB 访问。
- 确认安装后，脚本会将 \`ENABLE_WEB_UI\` 设置为 \`0\`，网页端将不可访问，统计数据改由 CLI 查看。
- 如需恢复网页访问，请重新运行安装脚本并选择 WEB 访问开关菜单。
- CLI 文件默认位于 \`/root/rustminersystem/RustMinerSystemCLI-0.1.0\`。
- 命令启动器默认位于 \`/usr/local/bin/rustminer\`。
`,"./zh/rustminersystem-cli/guide/02-configuration.md":`---
id: configuration
slug: configuration
title: CLI 模式修改 RustMinerSystem 配置
navTitle: 修改配置
description: 在 RustMinerSystem CLI 模式下编辑 rust-config、port-config.json 和 wallet-config.json，并通过 rustminer restart 使配置生效。
category: getting-started
categoryTitle: 开始使用
categoryOrder: 1
order: 2
...
`).map(e=>{let t=/^(#{2,3})\s+(.+)$/.exec(e.trim());return t?{id:Hr(t[2]),level:t[1].length,title:t[2]}:null}).filter(Boolean),Jr={id:`main-content`,class:`docs-shell`},Yr={class:`docs-sidebar`,"aria-labelledby":`docs-sidebar-title`},Xr={class:`docs-sidebar-inner`},Zr={class:`sidebar-groups`},Qr={class:`sidebar-nav`},$r={class:`docs-article`,"aria-labelledby":`document-title`},ei={class:`docs-header`},ti={class:`eyebrow`},ni={id:`document-title`},ri=[`innerHTML`],ii={class:`page-nav`,"aria-label":`Documentation page navigation`},ai={class:`article-aside`,"aria-labelledby":`article-toc-title`},oi={class:`article-aside-inner`},si={class:`aside-section`},ci={class:`aside-label`},li={class:`reading-time`},ui={class:`aside-section`},di={id:`article-toc-title`},fi={class:`article-toc`},pi=[`href`],mi=u({__name:`DocumentView`,setup(a){let{t:u}=l(),_=ie(),v=r(()=>te(_)),x=r(()=>_.meta?.docCollection||`rustminersystem`),S=r(()=>_.meta?.docPage||g),C=r(()=>se(x.value)),ae=r(()=>C.value.reduce((e,t)=>{let n=t.category||`guide`;return{...e,[n]:[...e[n]||[],t]}},{})),le=r(()=>re.filter(e=>e.collection===x.value&&ae.value[e.id]?.length)),ue=r(()=>oe(v.value,S.value,x.value)),w=r(()=>ue.value.replace(/^#\s+.+(?:\r?\n)+/,``)),de=r(()=>Kr(w.value)),fe=r(()=>qr(w.value)),T=r(()=>y(S.value,v.value,x.value).title),pe=r(()=>y(S.value,v.value,x.value).description),E=r(()=>ce(S.value,x.value)),me=r(()=>C.value[E.value-1]),he=r(()=>C.value[E.value+1]),ge=r(()=>{let e=w.value.replace(/```[\s\S]*?```/g,``).replace(/[#>*_`[\]()|-]/g,` `).trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.ceil(e/220))});return(r,a)=>(h(),p(`main`,Jr,[d(`aside`,Yr,[d(`div`,Xr,[d(`div`,Zr,[(h(!0),p(s,null,t(le.value,r=>(h(),p(`section`,{key:r.id,class:`sidebar-group`},[d(`h2`,null,f(c(ee)(r.id,v.value,x.value).title),1),d(`nav`,Qr,[(h(!0),p(s,null,t(ae.value[r.id]||[],t=>(h(),m(c(ne),{key:t.id,to:c(b)(t.id,v.value,x.value),class:o([`sidebar-link`,{"is-active":t.id===S.value}])},{default:n(()=>[e(f(c(y)(t.id,v.value,x.value).navTitle),1)]),_:2},1032,[`to`,`class`]))),128))])]))),128))])])]),d(`article`,$r,[d(`header`,ei,[d(`p`,ti,f(c(u)(`document.eyebrow`)),1),d(`h1`,ni,f(T.value),1),d(`p`,null,f(pe.value),1)]),d(`section`,{class:`document-content`,"aria-label":`Documentation content`,innerHTML:de.value},null,8,ri),d(`nav`,ii,[me.value?(h(),m(c(ne),{key:0,class:`page-nav-link`,to:c(b)(me.value.id,v.value,x.value)},{default:n(()=>[d(`span`,null,f(c(u)(`document.previousPage`)),1),e(` `+f(c(y)(me.value.id,v.value,x.value).navTitle),1)]),_:1},8,[`to`])):i(``,!0),he.value?(h(),m(c(ne),{key:1,class:`page-nav-link is-next`,to:c(b)(he.value.id,v.value,x.value)},{default:n(()=>[d(`span`,null,f(c(u)(`document.nextPage`)),1),e(` `+f(c(y)(he.value.id,v.value,x.value).navTitle),1)]),_:1},8,[`to`])):i(``,!0)])]),d(`aside`,ai,[d(`div`,oi,[d(`section`,si,[d(`p`,ci,f(c(u)(`document.readingTime`)),1),d(`p`,li,f(c(u)(`document.readingTimeValue`,{minutes:ge.value})),1)]),d(`section`,ui,[d(`h2`,di,f(c(u)(`document.onThisPage`)),1),d(`nav`,fi,[(h(!0),p(s,null,t(fe.value,e=>(h(),p(`a`,{key:e.id,class:o([`toc-link`,`toc-link-level-${e.level}`]),href:`#${e.id}`},f(e.title),11,pi))),128))])])])])]))}},[[`__scopeId`,`data-v-0f6403c6`]]);export{mi as default};