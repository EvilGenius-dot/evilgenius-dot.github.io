...
export const normalizeLocale = (locale) => (isSupportedLocale(locale) ? locale : DEFAULT_LOCALE)

// 从 vue-router route 中解析当前语言；没有语言参数时回退到默认英文。
export const getRouteLocale = (route) => normalizeLocale(route?.params?.locale || route?.meta?.locale)
...
export const pagePath = (page = 'home', locale = DEFAULT_LOCALE) => {
    ...
    const localePrefix = normalizedLocale === DEFAULT_LOCALE ? '' : `/${normalizedLocale}`
